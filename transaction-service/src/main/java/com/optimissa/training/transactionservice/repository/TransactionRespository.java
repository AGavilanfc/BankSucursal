...
public class TransactionRespository implements ITransactionRepository {
    ...
    @Override
    public List<Transaction> getAccountBylimits(int id , int page, int limit) {
        int offset = (page-1)*limit;
        String query = "SELECT * FROM TRANSACTION WHERE ACCOUNT_ID = ? LIMIT ? OFFSET ?";
        return jdbcTemplate.query(query, new BeanPropertyRowMapper<>(Transaction.class),id ,limit,offset);
    }
    ...
    @Override
    public List<Transaction> selectByStartWith(String select, String data) {
        return jdbcTemplate.query(
                "SELECT * FROM TRANSACTION WHERE "+ select +" LIKE ?",
